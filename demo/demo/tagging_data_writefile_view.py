...
def tagging_push(request):
	ctx = {}
	# 先将已有的labels存入字典中
	file_object = open('label_data/labels.txt','r', encoding='utf8')
	s = set()
	...
	file_object = open('label_data/word_list.txt','r', encoding='utf8')
	all_list = []
	for f in file_object:
	...
	if 'label' in request.GET and 'title' in request.GET:
		...
		if label != None:
			file_object = open('label_data/labels.txt','a', encoding='utf8')
			if title in s:
				print("该title已存在，冲突！")
			else:
